...
object Publisher {
  ...
  private val agent = Context.preloadFile("PublisherAgent", "/Users/paul/github/TopAigents/cornerstone/plan/publisher.AGENT.md")
  private val plan  = Context.preloadFile("PublisherPlan",  "/Users/paul/github/TopAigents/cornerstone/plan/publisher.md")
  ...
  private val systemMessage = ChatMessage(
    ...
    str(
      basePrompt.resultValue.cel +
        ...
        ("\r\rcornerstone/plan/publisher.AGENT.md contents:\r": Cel) + agent.resultValue.cel +
        ("\r\rcornerstone/plan/publisher.md contents:\r": Cel) + plan.resultValue.cel
    )
  )
  ...
  val workflow = Workflow(
    basePrompt +: spec +: agent +: plan +: CliEventHandler(
      systemMessage,
      runners = DepMap.depsFor("Publisher")
    ).steps
  )
  ...
  def apply(name: String, sessionId: Value[String], query: BaseValue[String], model: Field, background: Boolean): Call[RunWorkflowArgs[CliEventHandler.Input], ChatToolResponse] = {
    val args = RunWorkflowArgs(str("cornerstone-Publisher${WORKFLOW_ENV}"), obj(CliEventHandler.Input(sessionId, query, model, background = Value(background))))
  ...
  val runner: Tools.Runner = Tools.invokerRunner(
    name = "CornerstonePublisher",
    description = "Agent workflow for the Cornerstone Publisher plan; preloads SPEC.md, plan, and operating guide.",
    onQuery = query => apply(
      "query_cornerstone_publisher",
      str("cornerstone-CornerstonePublisher"),
      query,
