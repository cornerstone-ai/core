...
object FirestoreAndStorage {
  private val spec  = Context.preloadFile("CornerstoneSpec", "/Users/paul/github/TopAigents/cornerstone/SPEC.md")
  private val agent = Context.preloadFile("FirestoreAndStorageAgent", "/Users/paul/github/TopAigents/cornerstone/plan/firestore-and-storage.AGENT.md")
  private val plan  = Context.preloadFile("FirestoreAndStoragePlan",  "/Users/paul/github/TopAigents/cornerstone/plan/firestore-and-storage.md")
  ...
  private val systemMessage = ChatMessage(
    ...
    str(
      basePrompt.resultValue.cel +
        ...
        ("\r\rcornerstone/plan/firestore-and-storage.AGENT.md contents:\r": Cel) + agent.resultValue.cel +
        ("\r\rcornerstone/plan/firestore-and-storage.md contents:\r": Cel) + plan.resultValue.cel
    )
  )
  ...
  val workflow = Workflow(
    basePrompt +: spec +: agent +: plan +: CliEventHandler(
      systemMessage,
      runners = DepMap.depsFor("FirestoreAndStorage")
    ).steps
  )
  ...
  def apply(name: String, sessionId: Value[String], query: BaseValue[String], model: Field, background: Boolean): Call[RunWorkflowArgs[CliEventHandler.Input], ChatToolResponse] = {
    val args = RunWorkflowArgs(str("cornerstone-FirestoreAndStorage${WORKFLOW_ENV}"), obj(CliEventHandler.Input(sessionId, query, model, background = Value(background))))
  ...
  val runner: Tools.Runner = Tools.invokerRunner(
    name = "CornerstoneFirestoreAndStorage",
    description = "Agent workflow for the Cornerstone Firestore and Storage plan; preloads SPEC.md, plan, and operating guide.",
    onQuery = query => apply(
      "query_cornerstone_firestore_and_storage",
      str("cornerstone-CornerstoneFirestoreAndStorage"),
      query,
