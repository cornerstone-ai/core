...
object WorkflowsScala {
  private val spec  = Context.preloadFile("CornerstoneSpec", "/Users/paul/github/TopAigents/cornerstone/SPEC.md")
  private val agent = Context.preloadFile("WorkflowsScalaAgent", "/Users/paul/github/TopAigents/cornerstone/plan/workflows-scala.AGENT.md")
  private val plan  = Context.preloadFile("WorkflowsScalaPlan",  "/Users/paul/github/TopAigents/cornerstone/plan/workflows-scala.md")
  ...
  private val systemMessage = ChatMessage(
    ...
    str(
      basePrompt.resultValue.cel +
        ...
        ("\r\rcornerstone/plan/workflows-scala.AGENT.md contents:\r": Cel) + agent.resultValue.cel +
        ("\r\rcornerstone/plan/workflows-scala.md contents:\r": Cel) + plan.resultValue.cel
    )
  )
  ...
  val workflow = Workflow(
    basePrompt +: spec +: agent +: plan +: CliEventHandler(
      systemMessage,
      runners = DepMap.depsFor("WorkflowsScala")
    ).steps
  )
  ...
  def apply(name: String, sessionId: Value[String], query: BaseValue[String], model: Field, background: Boolean): Call[RunWorkflowArgs[CliEventHandler.Input], ChatToolResponse] = {
    val args = RunWorkflowArgs(str("cornerstone-WorkflowsScala${WORKFLOW_ENV}"), obj(CliEventHandler.Input(sessionId, query, model, background = Value(background))))
  ...
  val runner: Tools.Runner = Tools.invokerRunner(
    name = "CornerstoneWorkflowsScala",
    description = "Agent workflow for the Cornerstone Workflows (Scala) plan; preloads SPEC.md, plan, and operating guide.",
    onQuery = query => apply(
      "query_cornerstone_workflows_scala",
      str("cornerstone-CornerstoneWorkflowsScala"),
      query,
