...
object CostAndBudgets {
  private val spec  = Context.preloadFile("CornerstoneSpec", "/Users/paul/github/TopAigents/cornerstone/SPEC.md")
  private val agent = Context.preloadFile("CostAndBudgetsAgent", "/Users/paul/github/TopAigents/cornerstone/plan/cost-and-budgets.AGENT.md")
  private val plan  = Context.preloadFile("CostAndBudgetsPlan",  "/Users/paul/github/TopAigents/cornerstone/plan/cost-and-budgets.md")
  ...
  private val systemMessage = ChatMessage(
    ...
    str(
      basePrompt.resultValue.cel +
        ...
        ("\r\rcornerstone/plan/cost-and-budgets.AGENT.md contents:\r": Cel) + agent.resultValue.cel +
        ("\r\rcornerstone/plan/cost-and-budgets.md contents:\r": Cel) + plan.resultValue.cel
    )
  )
  ...
  val workflow = Workflow(
    basePrompt +: spec +: agent +: plan +: CliEventHandler(
      systemMessage,
      runners = DepMap.depsFor("CostAndBudgets")
    ).steps
  )
  ...
  def apply(name: String, sessionId: Value[String], query: BaseValue[String], model: Field, background: Boolean): Call[RunWorkflowArgs[CliEventHandler.Input], ChatToolResponse] = {
    val args = RunWorkflowArgs(str("cornerstone-CostAndBudgets${WORKFLOW_ENV}"), obj(CliEventHandler.Input(sessionId, query, model, background = Value(background))))
  ...
  val runner: Tools.Runner = Tools.invokerRunner(
    name = "CornerstoneCostAndBudgets",
    description = "Agent workflow for the Cornerstone Cost and Budgets plan; preloads SPEC.md, plan, and operating guide.",
    onQuery = query => apply(
      "query_cornerstone_cost_and_budgets",
      str("cornerstone-CornerstoneCostAndBudgets"),
      query,
