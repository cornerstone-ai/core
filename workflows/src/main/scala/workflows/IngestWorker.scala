...
object IngestWorker {
  ...
  private val agent = Context.preloadFile("IngestWorkerAgent", "/Users/paul/github/TopAigents/cornerstone/plan/ingest-worker.AGENT.md")
  private val plan  = Context.preloadFile("IngestWorkerPlan",  "/Users/paul/github/TopAigents/cornerstone/plan/ingest-worker.md")
  ...
  private val systemMessage = ChatMessage(
    ...
    str(
      basePrompt.resultValue.cel +
        ...
        ("\r\rcornerstone/plan/ingest-worker.AGENT.md contents:\r": Cel) + agent.resultValue.cel +
        ("\r\rcornerstone/plan/ingest-worker.md contents:\r": Cel) + plan.resultValue.cel
    )
  )
  ...
  val workflow = Workflow(
    basePrompt +: spec +: agent +: plan +: CliEventHandler(
      systemMessage,
      runners = DepMap.depsFor("IngestWorker")
    ).steps
  )
  ...
  def apply(name: String, sessionId: Value[String], query: BaseValue[String], model: Field, background: Boolean): Call[RunWorkflowArgs[CliEventHandler.Input], ChatToolResponse] = {
    val args = RunWorkflowArgs(str("cornerstone-IngestWorker${WORKFLOW_ENV}"), obj(CliEventHandler.Input(sessionId, query, model, background = Value(background))))
  ...
  val runner: Tools.Runner = Tools.invokerRunner(
    name = "CornerstoneIngestWorker",
    description = "Agent workflow for the Cornerstone Ingest Worker plan; preloads SPEC.md, plan, and operating guide.",
    onQuery = query => apply(
      "query_cornerstone_ingest_worker",
      str("cornerstone-CornerstoneIngestWorker"),
      query,
