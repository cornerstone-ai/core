...
object Frontend {
  ...
  private val agent = Context.preloadFile("FrontendAgent", "/Users/paul/github/TopAigents/cornerstone/plan/frontend.AGENT.md")
  private val plan  = Context.preloadFile("FrontendPlan",  "/Users/paul/github/TopAigents/cornerstone/plan/frontend.md")
  ...
  private val systemMessage = ChatMessage(
    ...
    str(
      basePrompt.resultValue.cel +
        ...
        ("\r\rcornerstone/plan/frontend.AGENT.md contents:\r": Cel) + agent.resultValue.cel +
        ("\r\rcornerstone/plan/frontend.md contents:\r": Cel) + plan.resultValue.cel
    )
  )
  ...
  val workflow = Workflow(
    basePrompt +: spec +: agent +: plan +: CliEventHandler(
      systemMessage,
      runners = DepMap.depsFor("Frontend")
    ).steps
  )
  ...
  def apply(name: String, sessionId: Value[String], query: BaseValue[String], model: Field, background: Boolean): Call[RunWorkflowArgs[CliEventHandler.Input], ChatToolResponse] = {
    val args = RunWorkflowArgs(str("cornerstone-Frontend${WORKFLOW_ENV}"), obj(CliEventHandler.Input(sessionId, query, model, background = Value(background))))
  ...
  val runner: Tools.Runner = Tools.invokerRunner(
    name = "CornerstoneFrontend",
    description = "Agent workflow for the Cornerstone Frontend plan; preloads SPEC.md, plan, and operating guide.",
    onQuery = query => apply(
      "query_cornerstone_frontend",
      str("cornerstone-CornerstoneFrontend"),
      query,
